...
if __name__ == '__main__':
    ...
    train_data = pd.read_pickle(root + lang + '/train/blocks.pkl').sample(frac=0.1, random_state=42)
    # test_data = pd.read_pickle(root + lang + '/test/blocks.pkl').sample(frac=0.1, random_state=42)
    ...
    for t in tqdm(range(1, categories+1), desc="Categories Progress"):
        if lang == 'java':
        ...
        # training procedure
        for epoch in tqdm(range(EPOCHS), desc=f"Training Epochs for Category {t}", leave=False):
            ...
            while i < len(train_data_t):
                ...
                # print(f"X1========= {train1_inputs}")
                # print(f"X2========= {train1_inputs}")
    ...
    # final_checkpoint_path = os.path.join(CHECKPOINT_DIR, "model_checkpoint.pth")
    # torch.save({
    #     'epoch': EPOCHS,
